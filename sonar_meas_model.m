function z_meas=sonar_meas_model(z_true,sonar_model)
...
z_max=sonar_model{1}.z_max;
sigma_hit=sonar_model{1}.sigma_hit;
lambda=sonar_model{1}.lambda_short;
sample=sonar_model{1}.sample;
...
%     i=ceil(length(sample)*rand(1,1));  %generate integer values from uniform set 1:length(sample)
    switch sample(i)
        case 1  % hit
            z_meas(j)=z_true(j)+sqrt(sigma_hit)*randn/15;
        case 2  % short
