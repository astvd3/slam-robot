function ztrue_out=ray_cast(walls, sonar_temp, robot_pose)

sense_angles=sonar_temp{1}.sonar_angles;
zmax=sonar_temp{1}.z_max;
...
%                 alphat(i)=atan2(sin(alphat_t),cos(alphat_t));
                ...
                if z_true_temp<=zmax
                    if xt>=(min(x1,x2)-1e-1) && xt <=(max(x1,x2)+1e-1) && yt>=(min(y1,y2)-1e-1) && yt<=(max(y1,y2)+1e-1) && ((abs(yt-m*xt-b)<1e-1) || vert)
